...
// DECLARE VARIABLES
    ...
    $ctrlf = "<img src='../img/ctrlf.png' title='alert level : severe' alt='alert level : severe' height='25'>";
    $red = "<img src='../img/red.png' title='alert level : severe' width='10' height='10'>&nbsp";
    $orange = "<img src='../img/orange.png' title='alert level : moderate' width='10' height='10'>&nbsp";
    $green = "<img src='../img/green.png' title='alert level : minor' width='10' height='10'>&nbsp";
    $one = "<img src='../img/one.png' width='20' height='20'>";
    $two = "<img src='../img/two.png' width='20' height='20'>";
    $three = "<img src='../img/three.png' width='20' height='20'>";
    $four = "<img src='../img/four.png' width='20' height='20'>";
    $rx_icon = "<img src='../img/rx_icon.png' title='NLM RxNorm' height='15'>";
    $dm_icon = "<img src='../img/dailymed_icon.png' title='NLM DailyMed' height='12'>";
    $db_icon = "<img src='../img/drugbank_icon.png' title='DrugBank' height='15'>";
    $atc_icon = "<img src='../img/atc_icon.png' title='ATC' height='15'>";
    $help1 = "
            <table>
            <tr><th style='text-align : right'>HOW IT WORKS <br><br><br></th><th>Search for drug interactions with other drugs, disease or gene variants, and find bespoke drug and disease alerts<br><br><br></th></tr>
            <tr><th style='text-align : right'><b>General Search&nbsp&nbsp&nbsp&nbsp:&nbsp&nbsp&nbsp </b></th><th style='font-weight:normal'>Note : Client details not required for a general search</th></tr>
            <tr><th></th><th style='font-weight:normal'>1. Select any drug combination - at least two for drug-drug interaction</th></tr>
            <tr><th></th><th style='font-weight:normal'>2. Select any disease combination - at least one for drug-disease combinations</th></tr>
            <tr><th></th><th style='font-weight:normal'>3. click <button type='button' disabled>Find Interactions</button><br><br></th></tr>
            <tr><th style='text-align : right'><b>Find existing client&nbsp&nbsp&nbsp&nbsp:&nbsp&nbsp&nbsp </b></th><th style='font-weight:normal'>1. Enter HBT ID, eg 10000, 10001, etc</th></tr>
            <tr><th></th><th style='font-weight:normal'>2. Click <button type='button' disabled>Find</button> to find client record</th></tr>
            <tr><th></th><th style='font-weight:normal'>3. Click on 'Client' tab below to view client record<br><br></th></tr>
            <tr><th style='text-align : right'><b>Add new client&nbsp&nbsp&nbsp&nbsp:&nbsp&nbsp&nbsp </b></th><th style='font-weight:normal'>1. Enter HBT ID</th></tr>
            <tr><th></th><th style='font-weight:normal'>2. Create Baseline for client record - select sex, age, drugs and diseases, and add notes if required</th></tr>
            <tr><th></th><th style='font-weight:normal'>3. Click <button type='button' disabled>Add</button> to add new client profile<br><br></th></tr>
            <tr><th style='text-align : right'><b>Update existing client&nbsp&nbsp&nbsp&nbsp:&nbsp&nbsp&nbsp </b></th><th style='font-weight:normal'>1. Find client as above</th></tr>
            <tr><th></th><th style='font-weight:normal'>2. Create Follow Up for client record - select new/changed drugs and diseases, and add notes if required</th></tr>
            <tr><th></th><th style='font-weight:normal'>3. Click <button type='button' disabled>Update</button> to update client profile</th></tr>
            <tr><th></th><th style='font-weight:normal'>4. View changes in 'Client' tab<br><br></th></tr>
            <tr><th style='text-align : right'><b>Add new Drugs or Diseases&nbsp&nbsp&nbsp&nbsp:&nbsp&nbsp&nbsp </b></th><th style='font-weight:normal'>Click <button type='button' disabled> + 1 </button> , select new drug/disease, then select A, B or C to refresh Drugs/Diseases tabs/summary :</th></tr>
            <tr><th></th><th style='font-weight:normal'>A. for a simple search, click <button type='button' disabled>Find Interactions</button></th></tr>
            <tr><th></th><th style='font-weight:normal'>B. to update existing client, click <button type='button' disabled>Update</button></th></tr>
            <tr><th></th><th style='font-weight:normal'>C. to add a new client, click <button type='button' disabled>Add</button><br><br></th></tr>
            <tr><th style='text-align : right'><b>Tips&nbsp&nbsp&nbsp&nbsp:&nbsp&nbsp&nbsp </b></th><th style='font-weight:normal'>Click <button type='button' disabled>CLS</button> to clear previous selections</th></tr>
            <tr><th></th><th style='font-weight:normal'>Select 'Choose One' in Drugs or Diseases list to remove item</th></tr>
            </table>";
...
function &getDiseasesList(){
    ...
    // FROM DATABASE (DRUG CODES TABLE), GET DRUG NAME AS STRING WHERE RX CODE = $rx
...
function &getDrugSourceIds($drug){
  $conn = connectSQLdb();
  $rx_icon = "<img src='../img/rx_icon.png' title='National Library Medicine RxNorm' height='15'>";
  $dm_icon = "<img src='../img/dailymed_icon.png' title='National Library Medicine DailyMed' height='12'>";
  $db_icon = "<img src='../img/drugbank_icon.png' title='DrugBank' height='15'>";
  $atc_icon = "<img src='../img/atc_icon.png' title='World Health Organisation' height='15'>";
  if ($conn->connect_error) {die("Connection failed: " . $conn->connect_error);}
  $sql = "SELECT * FROM `drug_codes` WHERE drug = '$drug'";
  $sqlquery = $conn->query($sql);
  if ($sqlquery->num_rows > 0) {
...
// FROM DATABASE (DRUG ALERTS TABLE), GET DRUG ALERTS WHERE RX CODE = $rx + FORMAT AS LONG STRING (ONE LINE PER CODE)
...
// FROM DATABASE (DAILY MED TABLE), GET DAILY MED PAGE SCRAPE AS LONG STRING WHERE SPL CODE = $spl + ADD DIV TAGS TO HTML
function &getDailyMedPage($spl){
$dm_icon = "<img src='../img/dailymed_icon.png' title='NLM DailyMed' width='20'>";
...
if ($sqlquery->num_rows > 0) {
  while($row = $sqlquery->fetch_assoc()) {
    $dailymed = $dailymed."<div style = 'font-size:10px; font-weight:normal; padding:30px; text-align:left'>".$row["dmcontent"]."</div>";}}
...
function &getDrugbankPage($drugbank_id){
    $db_icon = "<img src='../img/drugbank_icon.png' title='DrugBank' width='15'>";
    ...
    if ($sqlquery->num_rows > 0) {
      while($row = $sqlquery->fetch_assoc()) {
        $drugbank = $drugbank."<div style = 'font-size:10px; font-weight:normal; padding:30px; text-align:left'>".$row["dbcontent"]."</div>";}}
    else {$drugbank = $drugbank."<br>0 results";}
return $drugbank;}

// FROM DATABASE (DRUG INTERACTIONS TABLE), GET ALL DRUG INTERACTIONS AS LONG STRING WHERE RX CODE = $rx
...
// FROM DATABASE (DRUG INTERACTIONS TABLE), GET SELECT DRUG INTERACTIONS AS STRING WHERE RX CODES = $rx, $i_rx
...
// FROM DATABASE (DISEASE INTERACTIONS TABLE), GET DISEASE INTERACTION AS ARRAY WHERE DISEASE CODE = $icd AND RX CODE = $rx
function &getDiseaseInteractions($rx,$icd){
    ...
    $conn = connectSQLdb();
    if ($conn->connect_error) {die("Connection failed: " . $conn->connect_error);}
    $sql = "SELECT * FROM `dis_interactions` WHERE rxcui = '$rx' AND icd = '$icd'";
    $sqlquery = $conn->query($sql);
    if ($sqlquery->num_rows > 0) {
...
?><!DOCTYPE html><html>
<head><meta charset="utf-8"><title>Home Page</title><link href="02Style.css" rel="stylesheet" type="text/css"></head>
...
<table>
    ...
    // HBT ID field
    {?>HBT ID <input type="text" name="hbtid" id="hbtid" style="width:60px;" value = "<?php if (isset($_POST['clear'])) {echo "";} else {echo isset($_POST['hbtid']) ? $_POST['hbtid'] : '';}?>">
...
<tr><th>Height</th><th>
    ...
    <span style="color:#212021;font-weight:normal"> ft&nbsp&nbsp</span>
    ...
    <span style="color:#212021;font-weight:normal"> in&nbsp&nbsp</span>
</th></tr>
<tr><th>Weight</th><th>
    ...
    <span style="color:#212021;font-weight:normal"> st&nbsp&nbsp</span>
    ...
    <span style="color:#212021;font-weight:normal"> llb&nbsp&nbsp</span>
...
<?php # calculate BMI
    ...
    if (isset($_POST['clear'])) {$height=='0'; $weight=='0'; }
        ...
        {$bmi = round( $weight/($height*$height)*703, 1);
         echo "&nbsp&nbsp&nbsp&nbsp&nbspBMI <span style='color:#212021;font-weight:normal'>".$bmi." </span>";
...
<tr><th>Alcohol</th><th>
    ...
    <?php foreach($alcohol as $item){echo "<option value='$item'>$item</option>";} ?> </select <input name="alcohol" type="text"/><span style="color:#212021;font-weight:normal"> units pw</span>
</th></tr>
<tr><th>Smoking</th><th>
...
Notes<br><input type="text" name="notes" id="notes" style="width:250px;" onclick="submit_form(); value = "<?php echo isset($_POST['notes']) ? $_POST['notes'] : '' ?>"><br>

<?php }
// add spaces to stretch column with dropdown lists + number to avoid number being added to separate line //
?><span style="width:250px;">&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp</span>
</th></tr>
...
for ($j = 1; $j < $_SESSION['drugSelectorNr'] + 1; $j++)
    {?> <span style="color:#212021;font-weight:normal"><?php echo str_pad($j,2,'0', STR_PAD_LEFT);?> </span><?php # number drugs (with zero left of number if < 10)
...
for ($j = 1; $j < $_SESSION['diseaseSelectorNr'] + 1; $j++) {
    ?><span style="color:#212021;font-weight:normal"><?php echo str_pad($j,2,'0', STR_PAD_LEFT);?> </span><?php
...
$all_interactions = ''; $uniquelist = array();
    for ($j = 1; $j < $_SESSION['drugSelectorNr'] + 1; $j++) {
        for ($k = $j; $k < $_SESSION['drugSelectorNr'] + 1; $k++) {
            if ($drugs[$j] != "Choose one" && $drugs[$k] != "Choose one" && $drugs[$j] != $drugs[$k]) {
                $interaction_text = getSelectDrugDrugInteraction($rx[$j],$rx[$k]);
                $interaction_line =  $interaction_text[1]." ".$drugs[$j]." - ".$drugs[$k]." : ".$interaction_text[4];
...
if ($showhelp == 1) {echo $help1;} else {echo $summary;}

?>
</th></tr></table>
</form>
<?php // Javascript ?>
<script type="text/javascript" src="02Script.js"></script>
<?php
...
// DISPLAY CLIENT TAB
if ( ($_POST['hbtid'] != "") && (!isset($_POST['addDrug'])) && (!isset($_POST['addDisease'])) ) {?> <button class="tablinks" onclick="openList(event, 'Client')">Client<br><?php echo "<b>".$_POST['hbtid']."</b>" ?></button><?php;}
...
for ($j = 1; $j < $_SESSION['drugSelectorNr'] + 1; $j++)
    ...
    if ($tabdrug != "Choose one" && $tabdrug != "")
        {?> <button class="tablinks" onclick="openList(event, '<?php echo 'Drug '.$j; ?>')"><?php echo 'Drug #'.$j; ?><br><?php echo "<b>".$tabdrug."</b>"; ?></button><?php }
        else {if ($tabdrug != "Choose one" && $tabdrug != "") {?> <button class="tablinks" onclick="openList(event, '<?php echo 'drug '.$j; ?>')"><?php echo 'Drug #'.$j; ?><br><?php echo "<b>".$tabdrug."</b>"; ?></button><?php }}}
...
for ($j = 1; $j < $_SESSION['diseaseSelectorNr'] + 1; $j++)
    ...
    if ($tabdisease != "Choose one" && $tabdisease != "")
        {?> <button class="tablinks" onclick="openList(event, '<?php echo 'Disease '.$j; ?>')"><?php echo 'Disease #'.$j; ?><br><?php echo "<b>".$tabdisease."</b>"; ?></button><?php }
        else {if ($tabdisease != "Choose one" && $tabdisease != "") {?> <button class="tablinks" onclick="openList(event, '<?php echo 'disease '.$j; ?>')"><?php echo 'Disease #'.$j; ?><br><?php echo "<b>".$tabdisease."</b>"; ?></button><?php }}}
}?></div><?php
...
echo "<br><br><b>Log</b><br>".$shortbar.$client[52];
?></div><?php

// DISPLAY DRUG TABS CONTENT
# +1 drug/disease --> missing drug/disease content

for ($k = 1; $k < $_SESSION['drugSelectorNr'] + 1; $k++) { ?>
    <div id="<?php echo 'Drug '.$k; ?>" class="tabcontent">
    <?php
    if ($drugs[$k] != "Choose one") {
        ...
        for ($j = 1; $j < $_SESSION['drugSelectorNr'] + 1; $j++) {
            if ( ($drugs[$j] != "Choose one") && ($drugs[$j] != "") ){
                if ($drugs[$j] != $drugs[$k]) {
                    $interaction_text = getSelectDrugDrugInteraction($rx[$k],$rx[$j]);
                    echo "Drug #".$j." ".$interaction_text[1].$drugs[$j]." (Rx <a href=".$urlroot.$rx[$j].">".$rx[$j]."</a>) : ".$interaction_text[4]."<br>";}}}
        # interactions with all drugs
        echo "<br>Drug #".$k." <b>".$drugs[$k]."</b> with ALL ".count($allinteractions[$k])." drugs : <br>";
        $button = 'buttonid'.$k;
        ?>&nbsp&nbsp&nbsp&nbsp&nbsp<button class="button" onclick="showhide('<?php echo $button; ?>')">Click To View All</button><div id='<?php echo $button; ?>' style="display:none"><?php
        echo "<br><i>'Ctrl' + 'f' to find specific drug</i><br>";
        for ($j = 0; $j < count($allinteractions[$k]); $j++) {echo " #".($j+1)." ".$green." ".$allinteractions[$k][$j]."<br>";}
        echo "</div>";
        echo $shortbar.$dailymed[$k];
        #echo $shortbar.$drugbank_id[$k];
        ?>
</div>
    <?php }
    // DISPLAY DISEASE TABS CONTENT X 6
        for ($j = 1; $j < $_SESSION['diseaseSelectorNr'] + 1; $j++) { ?>
     <div id="<?php echo 'Disease '.$j; ?>" class="tabcontent">
        <?php
        $all_interactions = '';
        $icd = getDiseaseCode($diseases[$j]); if (!isset($icd)) {$icd = "0 results";} echo '<b>ICD : </b>'.$icd."&nbsp&nbsp-&nbsp&nbsp";
        ...
        for ($i = 1; $i < $_SESSION['diseaseSelectorNr'] + 1; $i++) {
            if ($drugs[$i] != "Choose one") {
                ...
                $interaction_text = getDiseaseInteractions($rx,$icd);
                $interaction_line =  $interaction_text[1]." ".$drugs[$i]." - ".$diseases[$j]." : ".$interaction_text[3]." - ".$interaction_text[4]." <i>(".$interaction_text[5].")</i><br>";
                if ($interaction_text[5] != '0 results') {$all_interactions = $all_interactions.$interaction_line; }}}
