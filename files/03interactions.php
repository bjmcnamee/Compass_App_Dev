...
// Javascript
?><script type="text/javascript" src="03Script.js"></script><?php
...
function &getGeneInteractions($drug){
    ...
    if ($sqlquery->num_rows > 0) {
        while($row = $sqlquery->fetch_assoc()) {
            ...
            $gene_alert = $colour." ".$drug." - Biomarker <b>".$row["biomarker"]."</b> + Genotype/Phenotype <b>".$row["snip"]."</b> , ".$source." <i>(".$row["version"].")</i><br>";
            //$gene_alert = $colour." ".$drug." - Biomarker <b>".$row["biomarker"]."</b> + Genotype/Phenotype <b>".$row["snip"]."</b> , ".$source." <i>(".$row["version"].")</i> <button class='button' onclick='showhide('buttonid')'>Actions</button><br>";
            ...
            $actions = $direct_action.$indirect_action."<br>";
            //$actions = "<div id='buttonid' style='display:none'>".$direct_action.$indirect_action."</div>";

            $gene_alerts = $gene_alerts.$gene_alert.$actions;
        }}
    return $gene_alerts;}
...
?><!DOCTYPE html><html>
<head><meta http-equiv="Content-Type" content="text/html; charset=utf-8"><title>Home Page</title><link href="03Style.css" rel="stylesheet" type="text/css"></head>
...
if( (!isset($_POST['hideClient'])) && (!isset($_POST['viewClient'])) ) {
    ...
    foreach ($drugs as $drug) {
        if ($drug != "Choose one") {
            $interaction_line = getGeneInteractions($drug);
        if ($interaction_line != "0 results") {$all_interactions = $all_interactions.$interaction_line;}}}
